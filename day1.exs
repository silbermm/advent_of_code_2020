defmodule Day1 do
  ...
  def three_numbers(first, [second | rest], original) do
    res =
      Enum.find(rest, fn x ->
        {a, _} = Integer.parse(x)
        {b, _} = Integer.parse(first)
        {c, _} = Integer.parse(second)
        a + b + c == 2020
      end)

    if res == nil do
      three_numbers(first, rest, original)
    else
      {a, _} = Integer.parse(res)
      {b, _} = Integer.parse(first)
      {c, _} = Integer.parse(second)
      IO.puts("Three Numbers: #{a * b * c}")
    end
  end
end
...
Day1.two_numbers(input)
Day1.three_numbers(input)
