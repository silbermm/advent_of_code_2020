defmodule Day1 do
  ...
  def check([current_head | current_lst]) do
    third(current_head, current_lst, current_lst)
  end

  def third(first, [], original_lst) do
    check(original_lst)
  end

  def third(first, [current_head | current_lst], original_lst) do
    res =
      Enum.find(current_lst, fn x ->
        {a, _} = Integer.parse(x)
        {b, _} = Integer.parse(current_head)
        {c, _} = Integer.parse(first)
        a + b + c == 2020
      end)

    if res == nil do
      third(first, current_lst, original_lst)
    else
      {a, _} = Integer.parse(res)
      {b, _} = Integer.parse(current_head)
      {c, _} = Integer.parse(first)
      IO.inspect(a * b * c)
    end
